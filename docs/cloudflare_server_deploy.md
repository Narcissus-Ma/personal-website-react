...
1. **server.mjs** - 从 Node.js HTTP 服务器转换为 Cloudflare Workers 格式
...
打开项目根目录下的 `wrangler.toml` 文件，将上一步获取的 `id` 和 `preview_id` 填入：
...
```bash
npm run worker:dev      # 本地开发 Worker
npm run worker:deploy    # 部署 Worker
npm run kv:init         # 初始化 KV 数据
npm run kv:get          # 获取 KV 数据
npm run kv:put          # 更新 KV 数据
```
...
- 检查 KV 使用情况：`wrangler kv:namespace list`
- 减小数据大小或升级到付费计划
