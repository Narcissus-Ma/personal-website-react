...
将项目整理为适合自行部署的个人网站导航模板：新用户能够快速了解功能、完成本地运行并按需部署到 Cloudflare；潜在贡献者能够按明确规则提交反馈和代码。此次发布不将现有管理 API 表述为生产级安全方案，而是如实标明其公开写入风险并把鉴权重构列为后续工作。
...
- `SECURITY.md`：支持范围、漏洞私下报告方式，以及不应在 Issue 中公开密钥或复现细节的规则；明确当前 Cloudflare Worker 的写入接口不适合暴露给不受信任的公网访问者。
...
- 新增 `cloudflare/wrangler.example.toml` 作为公开样板；将实际配置移至被 `.gitignore` 忽略的 `cloudflare/wrangler.toml`。调整所有 `cf:*` 脚本和包装器，使其优先使用私有配置，缺失时明确提示从样板复制；提交前不保留任何作者 Cloudflare 资源标识。
...
- 在 `README.md`、`SECURITY.md` 与部署指南中醒目说明安全边界：当前 Worker 的 `/api/save` 没有访问控制，`set-password` 不能保护写接口，默认 CORS 允许任意来源；部署者不得把它当作公开生产管理端。可信私有使用以外的场景，应等待后续鉴权与密码哈希改造或自行实现访问控制。

## 质量基线

- README/CONTRIBUTING 明确 Node.js 和 pnpm 的版本前置条件（以项目的锁文件与 `package.json` 实际兼容性为准）。
- 发布前依次执行已跟踪发布资产的敏感信息检查、Markdown 相对链接检查、`pnpm lint` 和 `pnpm build`，并将实际结果记录在发布 PR 中。
...
2. 作者专属 API 域名、KV ID 与私密配置均不出现在公开运行配置中，Cloudflare 私有配置可由公开样板建立且脚本使用路径明确。
3. 所有新增文档的相对内部链接均指向真实文件，且 Cloudflare 部署指引与当前代码、命令一致。
4. 许可证为 MIT，协作文档具备清晰的行为与安全反馈路径及部署安全边界说明，不把未鉴权写接口误述为受密码保护。
